...
nel caso che il vincolo sia attivo in $\overline{x}$ (cioé $g(\overline{x}) = 0$) può essere $\lambda_i \geq 0$. Altrimenti se il vincolo non è attivo ($g(\overline{x}) < 0$) deve essere $\lambda_i = 0$.

Per la verifica di ottimalità di $\overline{x}$ siamo passati da un controllo di infiniti vettori alla risoluzione di un sistema di equazioni che \`e trattabile. Risolvendo il sistema di Fritz--John si otterranno $\theta$ e dei $\lambda_i$. Questi ultimi varranno $0$ per i vincoli non attivi, fornendo un'indicazione si quali siano i vincoli che effettivamente entrano in gioco nella risoluzione del problema di minimizzazione. I $\lambda_i$ vengono chiamati \emph{moltplicatori di Lagrange}.
...
$$ \displaystyle \sum_{i \in I(\overline{x})}
  ( \overline{\lambda}_{i} - \hat{\lambda}) \nabla g_i(\overline{x}) = 0
$$
Allora
...
\end{itemize}
  ...
  Il fatto che si utilizzi un aperto e che la nuova funzione $f'(x)$ ai bordi vada a $+\inf$ ci garantisce che per trovare i punti stazionari $\overline{x}$ sia sufficiente verificare che
 $$ \nabla f'(\overline{x}) = 0$$
...
Sia $x(\epsilon)$ \`e punto di minimo di $(PB_{\epsilon})$.\\
Consideriamo il gradiente:
$$
\begin{array}{l}
 0 = \nabla q_{\epsilon}(x(\epsilon)) =
\nabla f(x(\epsilon)) - \epsilon
\displaystyle \sum_{i=1}^{n} \dfrac{1}{-g_i(x(\epsilon))}
\nabla g_i(x(\epsilon)) =  \\ 
\nabla f(x(\epsilon)) +
\displaystyle \sum_{i}^{n} \underbracket{
\left(\dfrac{-\epsilon}{g_i(x(\epsilon))}\right)}_{
\footnotesize{
\begin{array}{l}
\text{moltiplicatori} \\
\text{associati a } x(\epsilon) \\
\text{nel problema}\\
...
minimo locale $x(\epsilon)$ di $(PB_{\epsilon)})$ per $\epsilon$
sufficientemente piccolo
\item 
 $x(\epsilon) \xrightarrow{x \downarrow 0}x^{*}$ e $
\lambda_i(\epsilon) \xrightarrow{x \downarrow 0} \lambda_i^{*}$ sono i
moltiplicatori assiociati a $x^{*}$.
\end{itemize}
...
$$ g_1(x) = 2 - x_1$$
e quindi
$$ \nabla_{i}(\epsilon) =  \dfrac{-\epsilon}{g_i(x(\epsilon))}
= \dfrac{-\epsilon}{1- \sqrt{1+\epsilon} -1} \xrightarrow{\epsilon \downarrow 0 }
2
$$
...
%% REVIEW FROM HERE
 ...
 $x(\epsilon)$ e $\lambda_i(\epsilon) =  \dfrac{\epsilon}{-g_i(x(\epsilon))}$
soddisfano:
$$ 
\begin{array}{ll}
